...
function groupComplaints(records,station,roster){
  const m=new Map();
  for(const r of records.filter(x=>x.kind==='COMPLAINT'&&x.station===station)){
    ...
    if(!m.has(stable))m.set(stable,{name,station,complaints:0});
    m.get(stable).complaints++;
  }
...
function rescueRows(list){
  if(!list.length)return '<div class="home-empty">Sin datos para esta semana.</div>';
  return list.map((d,i)=>`<div class="home-rank-row"><div class="home-rank-num">${i+1}</div><div class="home-rank-driver"><b>${esc(d.name)}</b><span>${esc(d.station||'')}</span></div><div class="home-rank-value bad"><b>${d.rescueCount} rescates</b><span>${d.rescueStops} stops · ${d.rescuePackages} packages</span></div></div>`).join('');
}
...
async function refreshHome(){
  ...
  try{
    try{await syncHomeRescues({week:w});}catch(syncErr){console.warn('No pude refrescar LOG:',syncErr);}
    ...
    const c3=groupComplaints(records,'DJX3',roster),c4=groupComplaints(records,'DJX4',roster);
    ...
    root.innerHTML=`
      ...
      <div class="home-section-title"><div><span class="home-section-kicker danger">02 · ATTENTION</span><h3>Drivers con más complaints</h3></div><span class="home-pill danger">CDF · Top 10 por estación</span></div>
      <div class="home-grid-two">
        <div class="home-card"><div class="home-card-head"><div><span>DJX3</span><h4>Top 10 Complaints</h4></div><div class="home-medal">⚠️</div></div>${rankRows(c3,d=>String(d.complaints),'Complaints','bad')}</div>
        <div class="home-card"><div class="home-card-head"><div><span>DJX4</span><h4>Top 10 Complaints</h4></div><div class="home-medal">⚠️</div></div>${rankRows(c4,d=>String(d.complaints),'Complaints','bad')}</div>
      </div>
      ...
      <div class="home-grid-two">
        <div class="home-card"><div class="home-card-head"><div><span>DJX3</span><h4>Top 5 rescates negativos</h4></div><div class="home-medal">↓</div></div>${rescueRows(r3)}<div class="home-footnote">Cantidad de rescates, total de Stops y total de Packages recibidos.</div></div>
        <div class="home-card"><div class="home-card-head"><div><span>DJX4</span><h4>Top 5 rescates negativos</h4></div><div class="home-medal">↓</div></div>${rescueRows(r4)}<div class="home-footnote">Cantidad de rescates, total de Stops y total de Packages recibidos.</div></div>
      </div>`;
