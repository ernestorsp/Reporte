...
const DEFAULT_SCORING = {
  packages:0.15,rescueYes:-0.20,rescuePositive:0,ncns:0,co:0,lateMorning:0,
  complaints:0,safety:0,pickups:0,dsb:0,dvic:0,otherInfra:0
};
...
function canonicalCategory(v){
  const s=clean(v).toUpperCase().replace(/[_-]+/g,' ').replace(/\s+/g,' ');
...
function scoreRecord(r,scoring){
  ...
  if(r.kind==='RESCUE'){
    const affects=lower(r.extra?.affects);
...
function summarize(records,scoring){
  ...
  const out={packages,rescues:0,ncns:0,co:0,late:0,complaints:0,safety:0,pickups:0,dsb:0,dvic:0,total:packages*Number(scoring.packages||0)};
  for(const r of records.filter(x=>x.kind!=='OVERVIEW')){
    out.total+=scoreRecord(r,scoring);
    if(r.kind==='RESCUE')out.rescues++;
...
function buildPdf({name,transporterId,email,week,station,records,summary,scoring}){
  return new Promise((resolve,reject)=>{
    ...
    doc.moveDown(1.2);
    doc.roundedRect(48,178,516,70,8).fillAndStroke('#F8FAFC','#E4E7EC');
    ...
    const metrics=[
      ['Packages Delivered',summary.packages],['Rescues',summary.rescues],['NCNS',summary.ncns],['Call Out (CO)',summary.co],
      ['Late Morning',summary.late],['Complaints',summary.complaints],['Safety',summary.safety],['Pickups',summary.pickups],['DSB',summary.dsb],['DVIC',summary.dvic]
    ];
    let y=296;
    metrics.forEach((m,i)=>{
      const x=i%2===0?48:306; if(i%2===0&&i>0)y+=27;
      doc.font('Helvetica').fontSize(9).fillColor('#667085').text(m[0],x,y,{width:150});
      doc.font('Helvetica-Bold').fillColor('#101828').text(String(m[1]),x+165,y,{width:70,align:'right'});
    });

    y+=48;
    doc.font('Helvetica-Bold').fontSize(13).fillColor('#101828').text('Scoring detail',48,y); y+=22;
    const rows=[];
...
function transporter(){
  const user=REPORT_EMAIL.value();
  const pass=REPORT_EMAIL_APP_PASSWORD.value();
  if(!user||!pass) throw new HttpsError('failed-precondition','El correo remitente todavía no está configurado.');
  return {user,tx:nodemailer.createTransport({service:'gmail',auth:{user,pass}})};
}

async function loadScoring(){
  const s=await db.collection('settings').doc('scoring').get();
...
async function sendOne({week,station,transporterId}){
  ...
  await tx.sendMail({
    ...
    subject:`AAXI Xpress · Weekly Driver Report · ${week}`,
    html:`<div style="font-family:Arial,sans-serif;color:#101828;line-height:1.55"><h2 style="margin-bottom:4px">AAXI Xpress</h2><p>Hello ${name},</p><p>Your weekly performance report for <b>${week}</b> at <b>${station}</b> is attached as a PDF.</p><p><b>Total score: ${fmt(summary.total)}</b></p><p>Please review the attached report. If you have questions, reply to this email.</p><p style="color:#667085;font-size:12px">AAXI Xpress · Driver Performance Report</p></div>`,
    attachments:[{filename:`AAXI_Xpress_${week}_${transporterId}.pdf`,content:pdf,contentType:'application/pdf'}]
  });
...
exports.sendDriverReport = onCall({region:'us-east1',timeoutSeconds:120,memory:'512MiB',secrets:[REPORT_EMAIL,REPORT_EMAIL_APP_PASSWORD]},async request=>{
  ...
  return sendOne({week,station,transporterId});
});

exports.sendStationReports = onCall({region:'us-east1',timeoutSeconds:900,memory:'1GiB',secrets:[REPORT_EMAIL,REPORT_EMAIL_APP_PASSWORD]},async request=>{
  if(!request.auth)throw new HttpsError('unauthenticated','Acceso no preparado.');
  const week=clean(request.data?.week),station=clean(request.data?.station).toUpperCase();
  if(!/^\d{4}-W\d{2}$/.test(week)||!['DJX3','DJX4'].includes(station))throw new HttpsError('invalid-argument','Semana o estación inválida.');
  const snap=await db.collection('records').where('week','==',week).where('station','==',station).get();
  const ids=[...new Set(snap.docs.map(d=>d.data()).filter(r=>r.kind==='OVERVIEW').map(r=>clean(r.transporterId||r.driverKey)).filter(Boolean))];
  const sent=[],failed=[];
  for(const id of ids){
    try{sent.push(await sendOne({week,station,transporterId:id}));}
    catch(err){failed.push({transporterId:id,error:err.message||String(err)});}
  }
  return {ok:true,total:ids.length,sent:sent.length,failed:failed.length,failures:failed.slice(0,20)};
});
