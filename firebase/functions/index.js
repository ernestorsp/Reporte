...
// Kept temporarily so existing deployments do not need an immediate trigger deletion.
// New uploads use staging/... and are intentionally NOT processed here.
...
exports.generateWeek = onCall({region:'us-east1', timeoutSeconds:540, memory:'1GiB'}, async (request) => {
  ...
  await db.collection('generations').doc(week).set({
    week,status:'processing',startedAt:FieldValue.serverTimestamp(),documentCount:uploads.length,error:''
  },{merge:true});
  ...
  for (const u of uploads) {
    try {
      ...
      const [buffer]=await bucket.file(u.storagePath).download();
      ...
      const normalized=normalize(rows,{week,station:u.station,type:u.type,fileName:u.fileName});
      prepared.push({upload:u,records:normalized});
    } catch(err) {
  ...
  for (const p of prepared) {
    ...
    await db.collection('uploads').doc(p.upload.id).set({
      status:'generated',rows:p.records.length,validation:'OK',error:'',generatedAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()
    },{merge:true});
  }

  await db.collection('generations').doc(week).set({
    week,status:'generated',documentCount:prepared.length,records:totalRecords,error:'',generatedAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()
  },{merge:true});
...
function validate(headers,type){const normalized=headers.map(clean);const missing=[];for(const alternatives of REQUIRED[type]||[]){if(!alternatives.some(x=>normalized.includes(clean(x))))missing.push(alternatives[0]);}return missing.length?{ok:false,message:`Archivo incorrecto para ${type}. Faltan columnas: ${missing.join(', ')}`}:{ok:true};}
function truthy(v){const s=clean(v);return v===1||s==='1'||s==='yes'||s==='true'||s==='x';}
...
function normalize(rows,meta){
  ...
  for(const row of rows.slice(1)){
    ...
    if(meta.type==='OVERVIEW'){
      ...
      const date=get(row,idx,['date station local time','date (station local time)','date']);const metric=get(row,idx,['metric type']);if(metric)out.push(base(meta,key,name,transporterId,'INFRACTION',date,metric));
    } else if(meta.type==='CDF'){
      const date=get(row,idx,['delivery date','date']);const details=get(row,idx,['feedback details']);const skip=new Set(['week','delivery associate','driver','transporter id','transporterid','tracking id','delivery group id','feedback details','delivery date','station']);
      ...
      if(!truthy(get(row,idx,['impacts scorecard'])))continue;const date=get(row,idx,['delivery date','date']);const skip=new Set(['week','delivery associate','driver','transporter id','transporterid','tracking id','delivery group id','delivery date','impacts scorecard','station']);
      ...
      const failed=num(get(row,idx,['failed stops']));if(failed>0)out.push(base(meta,key,name,transporterId,'FAILED_PICKUP','','Failed Pickups',{count:failed}));
    } else if(meta.type==='DVIC'){
      const date=get(row,idx,['start date','start_date','date']);const inspection=get(row,idx,['inspection type','inspection_type'])||'Pre-Trip';const duration=num(get(row,idx,['duration']));out.push(base(meta,key,name,transporterId,'DVIC',date,inspection,{duration}));
    }
  }
  return out;
}
