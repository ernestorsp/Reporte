...
async function fetchLogSheet(sheetName){
  ...
  if(!res.ok){const body=await res.text();throw new Error(`No pude leer ${sheetName} desde Google Sheets (${res.status}). ${body.slice(0,180)}`);}
  const data=await res.json(),rows=Array.isArray(data.values)?data.values:[];
...
function parseFile(buffer,fileName){
  ...
  if(/\.xlsx?$/i.test(fileName)){const wb=XLSX.read(buffer,{type:'buffer',cellDates:true});return XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]],{header:1,defval:'',raw:false});}
...
function getAllByHeader(row,headers,name){const target=clean(name),vals=[];headers.forEach((h,i)=>{if(clean(h)===target&&row[i]!==''&&row[i]!=null)vals.push(row[i]);});return vals;}
...
function driverKey(name,transporterId){return String(transporterId||'').trim()||normalizeName(name).replace(/\s+/g,'_');}
...
function normalize(rows,meta){
  ...
  for(const row of rows.slice(1)){
    ...
    if(meta.type==='OVERVIEW'){
      const name=String(get(row,idx,['delivery associate'])||'').trim(),packages=num(get(row,idx,['packages delivered']));
      if(!name||packages===0)continue;
      const transporterId=String(get(row,idx,['transporter id','transporterid','transporter_id'])||'').trim(),key=driverKey(name,transporterId);if(!key)continue;
      out.push(base(meta,key,name,transporterId,'OVERVIEW','','',{standing:get(row,idx,['overall standing']),overallScore:num(get(row,idx,['overall score'])),packages}));continue;
    }
    ...
    if(meta.type==='SAFETY'){
      ...
      const categories=[];cleaned.forEach((h,i)=>{if(h&&!skip.has(h)&&truthy(row[i]))categories.push(String(headers[i]||'').trim());});
      const label=categories.length?categories.join(' · '):(details||'Customer Delivery Feedback');out.push(base(meta,key,name,transporterId,'COMPLAINT',date,label,{details,categories}));
    }else if(meta.type==='DSB'){
      if(!truthy(get(row,idx,['impacts scorecard'])))continue;const date=get(row,idx,['delivery date','date']);
      const skip=new Set(['week','delivery associate','driver','transporter id','transporterid','tracking id','delivery group id','delivery date','impacts scorecard','station']);cleaned.forEach((h,i)=>{if(h&&!skip.has(h)&&truthy(row[i]))out.push(base(meta,key,name,transporterId,'DSB',date,headers[i]));});
    }else if(meta.type==='PSB'){
      if(!truthy(get(row,idx,['impacts scorecard'])))continue;const date=get(row,idx,['pickup date','date']);const categories=['early arrivals','late arrivals','geo location','not attempted','contact compliance','out of return label','picked up, not returned'];let found=0;
      for(const category of categories){const i=idx[clean(category)];if(i!==undefined&&truthy(row[i])){out.push(base(meta,key,name,transporterId,'FAILED_PICKUP',date,headers[i],{count:1}));found++;}}if(!found)out.push(base(meta,key,name,transporterId,'FAILED_PICKUP',date,'Failed Pickup',{count:1}));
    }else if(meta.type==='DVIC'){
      const date=get(row,idx,['start date','start_date','date']),inspection=get(row,idx,['inspection type','inspection_type'])||'Pre-Trip',duration=num(get(row,idx,['duration']));out.push(base(meta,key,name,transporterId,'DVIC',date,inspection,{duration}));
    }
  }
  return out;
}
...
function parseLogRows({infra,rescues},meta,driverMap){
  const out=[];
  if(infra.length){const idx=idxMap(infra[0]);for(const row of infra.slice(1)){const name=String(get(row,idx,['driver'])||'').trim(),date=get(row,idx,['date']);if(!name||!dateBelongsToWeek(date,meta.week))continue;const match=driverMap.get(normalizeName(name));if(!match)continue;const category=String(get(row,idx,['category'])||'').trim(),affects=String(get(row,idx,['affects'])||'').trim();out.push(base({week:meta.week,station:match.station,type:'LOG',fileName:meta.fileName},match.key,match.name,match.transporterId,'LOG_INFRA',date,category,{category,severity:num(get(row,idx,['severity'])),affects,notes:String(get(row,idx,['notes'])||''),dispatcher:String(get(row,idx,['dispatcher'])||'')}));}}
  if(rescues.length){const idx=idxMap(rescues[0]);for(const row of rescues.slice(1)){const name=String(get(row,idx,['driver'])||'').trim(),date=get(row,idx,['date']);if(!name||!dateBelongsToWeek(date,meta.week))continue;const match=driverMap.get(normalizeName(name));if(!match)continue;const affects=String(get(row,idx,['affects'])||'').trim();out.push(base({week:meta.week,station:match.station,type:'LOG',fileName:meta.fileName},match.key,match.name,match.transporterId,'RESCUE',date,'Rescue',{stops:num(get(row,idx,['stop','stops'])),packages:num(get(row,idx,['packages'])),affects,notes:String(get(row,idx,['notes'])||''),dispatcher:String(get(row,idx,['dispatcher'])||'')}));}}
  return out;
}
...
function toDate(value){
  ...
  let m=s.match(/^(\d{1,2})[-\/.](\d{1,2})[-\/.](\d{4})$/);if(m)return new Date(Number(m[3]),Number(m[2])-1,Number(m[1]),12);
  m=s.match(/^(\d{4})[-\/.](\d{1,2})[-\/.](\d{1,2})$/);if(m)return new Date(Number(m[1]),Number(m[2])-1,Number(m[3]),12);
  const d=new Date(s);return Number.isNaN(d.getTime())?null:d;
}
function amazonWeekBounds(key){const m=String(key).match(/^(\d{4})-W(\d{2})$/);if(!m)return{start:new Date(0),end:new Date(0)};const year=Number(m[1]),week=Number(m[2]);const jan4=new Date(year,0,4,12),day=jan4.getDay()||7;const monday=new Date(jan4);monday.setDate(jan4.getDate()-(day-1)+7*(week-1));const start=new Date(monday);start.setDate(monday.getDate()-1);start.setHours(0,0,0,0);const end=new Date(start);end.setDate(start.getDate()+6);end.setHours(23,59,59,999);return{start,end};}
async function deleteRecords(week,station,type){const existing=await db.collection('records').where('week','==',week).where('station','==',station).where('sourceType','==',type).get();for(let i=0;i<existing.docs.length;i+=400){const batch=db.batch();existing.docs.slice(i,i+400).forEach(d=>batch.delete(d.ref));await batch.commit();}}
async function writeRecords(records){for(let i=0;i<records.length;i+=400){const batch=db.batch();records.slice(i,i+400).forEach(r=>batch.set(db.collection('records').doc(),{...r,createdAt:FieldValue.serverTimestamp()}));await batch.commit();}}
